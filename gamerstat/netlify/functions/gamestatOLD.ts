...
function clamp(value: number, minValue = 0, maxValue = 1) {
...
function calculateHeroConfidence(hero: HeroStats) {
...
function assignHeroPool(hero: HeroStats & { hero_confidence: number }) {
    const gamesPlayed = safeNumber(hero.games_played)
...
function calculateFlexRating(scoredHeroes: any[]) {
...
function assignFlexLabel(flexRating: number) {
    if (flexRating >= 80) {
...
function calculateRoleConfidence(role: RoleStats) {
...
function calculatePressureRating(general: Record<string, any>) {
    const winrate = safeNumber(general.winrate)
    const kda = safeNumber(general.kda)

    const deathsPerGame = safeNumber(general.average_deaths, kda)
...
function calculateReliabilityScore(general: Record<string, any>) {
...
function assignReliabilityLabel(score: number) {
    if (score >= 75) {
...
function createFallbackAiReview(summary: any) {
...
async function createAiReview(summary: any) {
...
function buildSummary(statsData: StatsData) {
    const general = statsData.general

    const scoredHeroes = Object.entries(statsData.heroes).map(
        ([heroName, heroStats]) => {
    ...
    const scoredRoles = Object.entries(statsData.roles).map(
        ([roleName, roleStats]) => {
            return {
                role: roleName,
    ...
    const heroPoolCounts = scoredHeroes.reduce<Record<string, number>>(
        (counts, hero) => {
